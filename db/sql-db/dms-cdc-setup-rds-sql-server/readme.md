# AWS DMS + CDC Setup on RDS SQL Server

Knowledge-transfer document for setting up AWS Database Migration Service (DMS) against Amazon RDS SQL Server sources using Change Data Capture (CDC).
...
### Why they specifically ask *us* (the DBA / systems team)
...
**Practical consequence:** every cross-account or cross-environment restore is followed by a DBA-led CDC re-enablement. Both layers need to be re-applied from scratch:
...
The consolidated setup script (`enable-cdc-v2.sql`) in this repo is designed exactly for this — re-running it after a restore is safe (idempotent) and re-establishes the full CDC state.
...
**Broader takeaway:** if your database has audit or compliance triggers on schema/security DDL events, check them *before* attempting `rds_cdc_enable_db` rather than after a mysterious failure — and treat the fix as something that needs to be reliable and repeatable, since any database that gets restored across environments will hit the same problem again.
