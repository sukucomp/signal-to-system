...
**1. The whole event becomes the message body.** The host serialises the
parsed event (all six device fields plus `received_at_utc`) as JSON and
sets it as the Service Bus message body. Both the Python and C# ingestors
emit snake_case JSON so consumers see identical bytes regardless of which
language produced the event.

**2. `event_id` becomes the Service Bus `MessageId`.** This is the
choice that makes Service Bus's built-in duplicate detection useful.
With the dedup window enabled on the queue (10 minutes by default in
this project), Service Bus collapses any retransmission of the same
MessageId into a single delivered message. The device-generated UUID
...
- **`read_rssi` and `antenna_port`** — these are UHF concepts. RSSI
  ...
  isn't) and no port to identify (there is one). They will be
  introduced in the post that moves to a UHF reader.
